...
install_post_merge_hook() {
    ...
    cat > "$hook_path" <<EOF
#!/bin/bash
# Restore web user ownership on EC-CUBE writable dirs after git pull.
# Generated by eccube-fim install.sh — do not edit manually.
WEB_USER="${WEB_USER}"
ECCUBE_ROOT="${ECCUBE_ROOT}"
for dir in var html/upload app/Plugin app/PluginData; do
    [ -d "\$ECCUBE_ROOT/\$dir" ] && chown -R "\$WEB_USER:\$WEB_USER" "\$ECCUBE_ROOT/\$dir"
done
EOF
    chmod 700 "$hook_path"
